...
class AdventureGameTests(unittest.TestCase):
    ...
    def test_loop_exists(self):
        subtest_message = "SubTest: Checking for a 'for' or 'while' loop in the code..."
        try:
            with self.subTest(subtest_message):
                has_loop = any((isinstance(node, ast.For) or isinstance(node, ast.While)) for node in ast.walk(self.tree))
                self.assertTrue(
                    has_loop,
                    msg_color("No 'for' or 'while' loop found in the code.", "red")
                )
        except AssertionError as ae:
            self.fail(msg_color(f"Assertion error occurred: {ae}", "red"))
        
    def test_if_name_main_exists(self):
        subtest_message = "SubTest: Checking for 'if __name__ == \"__main__\"'..."
        try:
            with self.subTest(subtest_message):
                found_main_guard = re.search(
                    r'if\s+__name__\s*==\s*[\'"]__main__[\'"]\s*:', self.file_content)
                self.assertTrue(
                    found_main_guard,
                    msg_color("'if __name__ == \"__main__\"' block is missing.", "red")
                )
        except AssertionError as ae:
            self.fail(msg_color(f"Assertion error occurred: {ae}", "red"))
        
    def test_two_or_more_user_choices(self):
        subtest_message = "SubTest: Checking if user has at least two choices..."
        try:
            with self.subTest(subtest_message):
                choice_patterns = re.findall(r'input\([^)]+\)', self.file_content)
                
                self.assertGreaterEqual(
                    len(choice_patterns), 2,
                    msg_color("Less than two user input choices detected.", "red")
                )
        except AssertionError as ae:
            self.fail(msg_color(f"Assertion error occurred: {ae}", "red"))

    def test_game_end_conditions(self):
        subtest_message = "SubTest: Checking for win/loss or game over messages..."
        try:
            with self.subTest(subtest_message):
                win_loss_phrases = ["victorious", "you have been", "game over", "play again", "thanks for playing"]
                found = any(phrase in self.file_content.lower() for phrase in win_loss_phrases)
                self.assertTrue(
                    found,
                    msg_color("No win/loss or game over messages found in the game.", "red")
                )
        except AssertionError as ae:
            self.fail(msg_color(f"Assertion error occurred: {ae}", "red"))
            
    def test_file_name_is_python(self):
        subtest_message = "SubTest: Checking if target file is a Python file (.py)..."
        try:
            with self.subTest(subtest_message):
                self.assertTrue(
                    file_name.endswith(".py"),
                    msg_color("Target file is not a Python (.py) file.", "red")
                )
        except AssertionError as ae:
            self.fail(msg_color(f"Assertion error occurred: {ae}", "red"))
